...
class BacktestEngine:
    ...
    def __init__(self, initial_equity: float = 1000.0, risk_fraction: float = 0.01) -> None:
        ...
        for idx in range(60, min_len):
            ...
            if side == "HOLD":
                if simulator.open_trade:
                    closed = simulator.close_position(price, timestamp, reason="HOLD_EXIT")
                    if closed:
                        equity += closed.pnl
            ...
            if simulator.open_trade and simulator.open_trade.side != side:
                closed = simulator.close_position(price, timestamp, reason="SIGNAL_FLIP")
                if closed:
                    equity += closed.pnl
            if simulator.open_trade is None:
                simulator.open_position(side=side, quantity=qty, price=price, timestamp=timestamp)
            equity_curve.append(equity)

        if simulator.open_trade and len(df5):
            last = df5.iloc[-1]
            closed = simulator.close_position(float(last["close"]), last["timestamp"].to_pydatetime(), reason="END_OF_BACKTEST")
            if closed:
                equity += closed.pnl
                equity_curve.append(equity)
