...
class Database:
    ...
    async def _ensure_database_and_role(self, app_url: URL) -> None:
        ...
        try:
            ...
            if exists:
                if db_name == 'postgres':
                    raise RuntimeError('Se solicitó recrear la base "postgres", operación no permitida por seguridad.')
                ...
                await conn.execute(f'DROP DATABASE "{escaped_db_name}"')
                logger.warning(f'Base de datos "{db_name}" eliminada para recreación limpia de arranque.')
