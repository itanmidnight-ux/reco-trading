...
class QuantKernel:
    ...
    def _ensure_runtime_state_fields(self) -> None:
        required_fields = ('negative_edge_streak', 'binance_min_notional', 'final_order_notional')
        if all(hasattr(self.state, field_name) for field_name in required_fields):
            return

        # Compatibilidad con estados legacy con slots fijos: recreamos RuntimeState
        # preservando los atributos existentes y completando defaults seguros.
        replacement = RuntimeState()
        for field_name in RuntimeState.__dataclass_fields__:
            if hasattr(self.state, field_name):
                setattr(replacement, field_name, getattr(self.state, field_name))
