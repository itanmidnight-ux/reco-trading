...
class QuantKernel:
    ...
    def _should_activate_kill_switch(self) -> tuple[bool, str]:
        ...
        if self.initial_equity > 0:
            ...
            daily_pnl = self.state.realized_pnl + self.state.unrealized_pnl - self.state.fees_paid
            if daily_pnl <= -(self.initial_equity * self.s.max_daily_loss):
                self.state.kill_switch = True
        ...
        try:
            ...
            while not self.shutdown_event.is_set():
                try:
                                    ...
                                    base_friction_cost = float((2.0 * self.s.taker_fee) + (self._last_market_quality.spread_bps / 10_000.0) + (self.s.slippage_bps / 10_000.0))
                                    spread_penalty = float(np.clip(self._last_market_quality.spread_bps / max(self.s.market_max_spread_bps, 1e-6), 0.0, 2.0) * 0.0005)
                                    friction_cost = float((base_friction_cost * 1.35) + spread_penalty)
                                    ...
                                    if self.state.position_state == PositionState.LONG:
                                        ...
                                        if self.state.tp_price > 0 and last_price >= self.state.tp_price:
                                            ...
                                            unrealized = (last_price - self.state.entry_price) * self.state.position_qty
                                            est_close_fee = (self.state.position_qty * last_price) * float(self.s.taker_fee)
                                            if unrealized > est_close_fee:
                                                decision = 'SELL'
                                                self.activity_text = f'Cierre por tiempo con ganancia neta ({elapsed:.0f}s)'
                                    ...
                                    if decision == 'BUY':
                                        if self.operating_mode == 'MINIMAL':
                                            equity_cap_notional = max(self.state.equity * 0.25, 0.0)
                                            minimal_trade_notional = max(self.s.minimal_fixed_position_notional, self.s.minimal_absolute_risk_usdt)
                                            requested_notional = max(0.0, min(minimal_trade_notional, equity_cap_notional))
