...
class RiskManager:
    ...
    def evaluate_kill_switch(self, drawdown: float, latency_ms: float, spread_bps: float, execution_error: bool = False) -> tuple[bool, str]:
    ...
    def validate_trade(self, *, drawdown: float, latency_ms: float, spread_bps: float, equity: float, confidence: float, atr: float) -> RiskDecision:
        blocked, reason = self.evaluate_kill_switch(drawdown, latency_ms, spread_bps)
        if blocked:
