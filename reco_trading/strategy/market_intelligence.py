...
class VolatilityFilter:
    """Classifies volatility and proposes risk adjustment. Optimized for crypto."""
    ...
    def evaluate(self, df: pd.DataFrame) -> VolatilityAssessment:
        ...
        if adx < 18 and atr_ratio < 0.003:
            return VolatilityAssessment(VolatilityState.LOW_VOLATILITY, allow_trade=False, risk_multiplier=0.0)
        if atr_ratio < 0.0025 and candle_range_pct < 0.003 and recent_vol < 0.003 and adx < 15:
            return VolatilityAssessment(VolatilityState.LOW_VOLATILITY, allow_trade=True, risk_multiplier=0.45)
        if atr_ratio < 0.0045 and candle_range_pct < 0.006:
            return VolatilityAssessment(VolatilityState.LOW_VOLATILITY, allow_trade=True, risk_multiplier=0.60)
        if atr_ratio < 0.0075 and candle_range_pct < 0.009:
            return VolatilityAssessment(VolatilityState.NORMAL_VOLATILITY, allow_trade=True, risk_multiplier=0.80)
        if atr_ratio > 0.025 or candle_range_pct > 0.030 or recent_vol > 0.025:
            return VolatilityAssessment(VolatilityState.EXTREME_VOLATILITY, allow_trade=True, risk_multiplier=0.50)
        return VolatilityAssessment(VolatilityState.NORMAL_VOLATILITY, allow_trade=True, risk_multiplier=1.0)
