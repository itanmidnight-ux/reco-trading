...
if [[ ! -f config/database.env ]]; then
  echo "Error: no se encontró config/database.env" >&2
  exit 1
fi
...
case "${DB_HOST}" in
  ...
  *)
    echo "Error: scripts/ensure_postgres.sh solo puede auto-configurar PostgreSQL local (DB_HOST actual: ${DB_HOST})." >&2
    exit 1
    ;;
esac
...
ensure_postgres_service() {
  ...
  if ! command -v pg_lsclusters >/dev/null 2>&1; then
    echo "Error: pg_lsclusters no está disponible. Ejecuta ./install.sh para instalar PostgreSQL." >&2
    exit 1
  fi
}
